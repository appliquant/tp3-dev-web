...
3. Modifier les valeurs dans `api/.env`
   ...
   2. Changer `DATA_BASE` pour le lien de la base de données (ex: DB_URL=mongodb://localhost:27017/taches)
...
3. Pour lancer l'application localement, changer les valeurs dans `public/taches-app/.env`
   1. Changer `VITE_API_URL` pour le lien de l'API (ex: VITE_API_URL=localhost:3000/) (avec un **/** à la fin)
